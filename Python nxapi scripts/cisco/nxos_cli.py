...
def _bcm_sdk_shell_all_unit(str=""):
    s = str.strip()
    if s:
        o = ""
        # Pick first online LC and first online FM
        # Symmetric programming makes it sufficient to collect data from one of each
        mods = []
        for i in range(0, 15):
        ...
        for lc in mods: 
            for unit in units:
                #print "%s : %s : %s" % (lc, unit, s)
                o += runBcmCmd(lc, unit, s)
        st = 0
        #print o
    return st, o

def _bcm_sdk_shell(str=""):
    s = str.strip()
    if s:
        st, o = _cisco.exec_hardware(s)
    return st, o
