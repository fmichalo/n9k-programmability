...
class RouteMap(object):
    ...
    def match_ip_access_list(self, lists, **kwargs):
        ...
        Arguments:
            lists: A string which is a space separated list of IP access list 
                names.
    ...
    def match_ipv6_access_list(self, list, **kwargs):
        ...
        Arguments:
            lists: A string which is a space separated list of IPV6 access list 
                names.
    ...
    def match_route_type(self, **kwargs):
        ...
        Optional Arguments:
            external: A boolean, set to True to match external routes (BGP, 
                EIGRP and OSPF type 1/2)
            internal: A boolean, set to True to match internal routes (including
            ...
            no: A boolean, set to True to stop matching the route-type of any 
        ...
        if not (kwargs.has_key('external') or kwargs.has_key('internal') or 
                kwargs.has_key('level_1') or kwargs.has_key('level_2') or 
                kwargs.has_key('local') or kwargs.has_key('nssa-external') or
                kwargs.has_key('type_1') or kwargs.has_key('type_2')):
            raise AttributeError, 'Must specify at least one route type'
        cmd += NXCLI._read_arg_from_dict(external, 'external', ' external', 
                {boolean: None})
        cmd += NXCLI._read_arg_from_dict(internal, 'internal', ' internal',
                {boolean: None})
        cmd += NXCLI._read_arg_from_dict(level_1, 'level_1', ' level-1', 
                {boolean: None})
        cmd += NXCLI._read_arg_from_dict(level_2, 'level_2', ' level-2', 
                {boolean: None})
        cmd += NXCLI._read_arg_from_dict(local, 'local', ' local', 
                {boolean: None})
        cmd += NXCLI._read_arg_from_dict(nssa_external, 'nssa_external', 
                ' nssa-external', {boolean: None})
        cmd += NXCLI._read_arg_from_dict(type_1, 'type_1', ' type-1', 
                {boolean: None})
        cmd += NXCLI._read_arg_from_dict(type_2, 'type_2', ' type-2', 
                {boolean: None})
    ...
    def set_forwarding_address(self, **kwargs):
        ...
        cmd = NXCLI._add_no_if_present('set forwarding-address')
        return self._run_cfg_at_routemap_level(cmd)
