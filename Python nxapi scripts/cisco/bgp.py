...
class BGPSession(Feature):
    ...
    def _load_all_existing_configs(self):
        status, output = cli('show running-config bgp')
        rows = output.split('\n')
        for row in rows:
    ...
    def is_shutdown(self):
        ...
        if self.is_enabled():
            status, output = cli('show run bgp | include shutdown')
            rows = output.split('\n')
            for row in rows:
    ...
    def cfg_aggregate_address(self, address, **kwargs):
        ...
        cmd = ""
        parsed_ip = ipaddress.ip_network(address, strict = False)
        if parsed_ip:
            cmd += NXCLI._add_no_if_present('aggregate-address %s' % 
                    str(parsed_ip), kwargs)
    ...
    def add_network(self, network, **kwargs):
        ...
        parsed_ip = ipaddress.ip_network(network, strict = False)
        if parsed_ip:
            cmd = NXCLI._add_no_if_present('network %s' % str(parsed_ip), kwargs)
    ...
    class BGPNeighbor(object):
        ...
        def _run_cfg_at_ngbr_level(self, cmds):
            string = 'router bgp %d' % self.session_ASN
            if self.vrf.get_name() != 'default':
                if not self.vrf.exists():
                    self.vrf.create()
                string += ' ; vrf %s' % self.vrf.get_name()
