...
class RunQueryView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if data_requested == 'hostnames':
            ...
            elif data_requested == 'foostats':
                from foo.tasks import poll_foostats
                from foo.admin import FooStats
                poll_foostats.delay(hostname=hostname)
                hs = FooStats.objects.filter(hostname__hostname=hostname)
                last_timestamp = hs.latest('polling_timestamp').polling_timestamp
                last_poll = hs.filter(polling_timestamp=last_timestamp)
                foodata = last_poll.values_list('stat1', 'stat2')
                foodata = [list(i) for i in foodata]
                context['data'] = foodata
