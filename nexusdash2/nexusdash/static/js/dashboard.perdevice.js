...
dashboard.getFooStats = function () {
    moduleData("foostats", function (data) {
        destroy_dataTable("table-foostats");
        $("#filter-foostats").val("").off("keyup");

        var modstatsTable = $("#table-foostats").dataTable({
            aaData: data.data,
            aoColumns: [
                { sTitle: "Stat1" },
                { sTitle: "Stat2" }
        ...
        $("#filter-foostats").on("keyup", function () {
            modstatsTable.fnFilter(this.value);
        });
    });
}
...
dashboard.fnMap = {
    ...
    foostats: dashboard.getFooStats,
};
