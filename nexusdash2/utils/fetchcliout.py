...
class FetchViaNxapi(FetchCliOut):
    ...
    def get_foostats(self):
        '''
        @return: dict d with keys matching exactly as 
        attributes of foo.models.FooStats
        '''
        l = list()
        if self._is_online:
            # TODO: Change the command and column name as per your need
            o = json.loads(NXAPITransport.clid('show clock'))
            try:
                d = dict()
                d['stat1'] = 'foo: nxos time: {0}'.format(str(o["simple_time"]))
                d['stat2'] = 'bar: local time: {0}'.format(time.asctime())
                l.append(d)
            except Exception as e:
                print 'Error while get_foostats', e
        return l
...
#     
#     fetcho = FetchViaNxapi(target_url='http://10.201.30.194/ins', username='CISCO\\ambarik', password='xxxx')
...
#     print 'get_foostats', fetcho.get_foostats()
#     print 'test'
